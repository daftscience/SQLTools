...
class Settings:
    ...
    def getConnections():
        ...
        options = sublime.load_settings(
            Const.CONNECTIONS_FILENAME).get('connections')
...
class Connection:
    ...
    def __init__(self, name, options):
        ...
        self.cli = sublime.load_settings(
            Const.SETTINGS_FILENAME).get('cli')[
            options['type']]
        cli_path = shutil.which(self.cli)

        if cli_path is None:
            sublime.message_dialog(
                "'{0}' could not be found by Sublime Text.\n\nPlease set the '{0}' path in your SQLTools settings before continue.".format(
                    self.cli))
            return

        self.rowsLimit = sublime.load_settings(
            Const.SETTINGS_FILENAME).get('show_records').get(
            'limit', 50)
    ...
    def loadDefaultConnectionName():
        default = sublime.load_settings(
            Const.CONNECTIONS_FILENAME).get(
            'default', False)
        if not default:
            return
    ...
    def getTables(self, callback):
        ...
        self.runCommand(
            self.builArgs('desc'), query,
            lambda result: Utils.getResultAsList(result, callback))

    def getColumns(self, callback):
        try:
            query = self.getOptionsForSgdbCli()['queries']['columns']['query']
            self.runCommand(
                self.builArgs('columns'),
                query,
                lambda result: Utils.getResultAsList(
                    result,
                    callback))
        except Exception:
            pass
    ...
    def getTableRecords(self, tableName, callback):
        ...
        self.runCommand(
            self.builArgs('show records'),
            query,
            lambda result: callback(result))
...
class Command(threading.Thread):
    ...
    def stop(self):
        ...
        try:
            ...
            self.process = None
            sublime.message_dialog(
                "Your command is taking too long to run. Try to run outside using your database cli.")
...
class History:
    ...
    def add(query):
        if len(
                History.queries) >= sublime.load_settings(
                Const.SETTINGS_FILENAME).get(
                'history_size',
                100):
